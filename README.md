<div align="center">
  ...
  <p>
    ...
    <img alt="Regression tests: 257" src="https://img.shields.io/badge/tests-257-3b6ea8">
...
| Regression tests | `257` | Unit and workflow tests run before publishing the current page. |
...
python bench/publish_metrics.py --output docs/data/benchmarks.json --tests 257
```
...
When strict BM25 finds no rows, search can retry with a conservative relaxed
query built from informative terms that co-occur in the best internal candidate.
Stopwords and no-signal terms are not eligible survivor terms, and stopword-only
queries abstain. Relaxation does not turn separately matching terms into a broad
OR query. `--json --explain` reports this under `query_diagnostics`.
...
apollokairn vocab lookup "k8s rollback" --path PATH_TO_VAULT --json
apollokairn vocab suggest "kubernetes rollback" --path PATH_TO_VAULT --json
apollokairn vocab validate --path PATH_TO_VAULT
```

`vocab lookup` inspects approved glossary entries that match a query. `vocab
suggest` is an authoring aid for proposing new aliases from local note evidence.
